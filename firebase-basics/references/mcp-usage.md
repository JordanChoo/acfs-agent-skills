...
1.  **Locate MCP configuration**
    ...
    For **Claude Code**, the MCP config lives under `~/.claude/` (typically
    `~/.claude/mcp_servers.json` or merged into `~/.claude/settings.json`
    depending on harness version). Detect first; do not assume a path.
