...
In this environment, the deeper rules-test-before-deploy discipline lives in
`firebase-stack/SKILL.md` ("Editing security rules"). Google's
`firebase/agent-skills` bundle also ships a `firebase-security-rules-auditor`
skill — opt-in install via `npx -y skills add firebase/agent-skills -y`.
